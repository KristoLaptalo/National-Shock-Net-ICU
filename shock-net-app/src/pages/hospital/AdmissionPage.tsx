/**
 * ICU Admission Page
 * Form for admitting approved patients to ICU
 */
...
export function AdmissionPage() {
  ...
  const onSubmit = async (data: unknown) => {
    ...
    try {
      // Bundle admission data for tracking update
      const admissionNote = {
        type: 'admission',
      ...
      // Call Supabase RPC to update tracking with admission data
        ...
        <Card>
          <CardContent className="text-center py-8">
            <div className="w-16 h-16 bg-shock-teal-light rounded-full flex items-center justify-center mx-auto mb-4">
              <svg
                className="w-8 h-8 text-shock-teal"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <div className="mb-6">
              <AnonymizationBadge code={MOCK_PATIENT.tt} size="lg" />
            </div>
            <div className="flex gap-3 justify-center">
              <Button onClick={() => navigate(`${ROUTES.HOSPITAL.DAILY_ENTRY}/${patientId || ''}`)}>
                Start Daily Entry
              </Button>
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Complete the admission form for approved patients
          </p>
        </div>
      </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm text-gray-500">
                  {MOCK_PATIENT.ageBracket} y/o {MOCK_PATIENT.sex === 'M' ? 'Male' : 'Female'}
                </p>
                <p className="font-medium">
                  {MOCK_PATIENT.shockType} Shock - SCAI {MOCK_PATIENT.scaiStage}
                </p>
              </div>
            </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <FormSection
          ...
          description="ICU unit and bed allocation"
        >
          <FormRow>
            <Select
              label="ICU Unit"
        ...
        <FormSection
          title="Care Team"
          description="Assigned physicians and nurses"
        >
          <FormRow>
            <Input
              label="Attending Physician"
              placeholder="Dr. ..."
              error={errors.attendingPhysician?.message}
              required
        ...
        <FormSection
          ...
          description="Date, time, and source of admission"
        >
          <FormRow>
            <Input
              label="Admission Date/Time"
              type="datetime-local"
              error={errors.admissionDateTime?.message}
              required
              {...register('admissionDateTime')}
            />
            <Select
              label="Admission Source"
              options={ADMISSION_SOURCE_OPTIONS}
              placeholder="Select source"
              error={errors.admissionSource?.message}
              required
        ...
        <FormSection
          ...
          description="Admission severity scores"
        >
          <FormRow>
            <Input
              label="GCS Score"
              ...
              max={15}
              placeholder="3-15"
              error={errors.initialGCS?.message}
              {...register('initialGCS', { valueAsNumber: true })}
            />
            <Input
              label="SOFA Score"
              type="number"
              ...
              max={24}
              placeholder="0-24"
              error={errors.initialSOFA?.message}
              {...register('initialSOFA', { valueAsNumber: true })}
            />
          </FormRow>
        </FormSection>
        ...
        <FormSection
          ...
          description="Lines and catheters in place"
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Checkbox
              label="Arterial Line"
              description="For continuous BP monitoring"
              {...register('hasArterialLine')}
            />
            <Checkbox
              label="Central Line"
              description="Central venous catheter"
              {...register('hasCentralLine')}
            />
            <Checkbox
              label="PA Catheter"
              description="Swan-Ganz catheter"
              {...register('hasPAC')}
            />
          </div>
        </FormSection>

        {/* Vasopressors Section */}
        <FormSection
          title="Vasopressor Support"
          description="Current vasopressor therapy"
        >
          <Checkbox
            label="On Vasopressors"
            description="Patient is currently receiving vasopressor support"
            {...register('onVasopressors')}
          />
          {onVasopressors && (
            <div className="mt-4">
              <Textarea
                label="Vasopressor Details"
                placeholder="e.g., Norepinephrine 0.1 mcg/kg/min, Vasopressin 0.04 units/min"
                fullWidth
                {...register('vasopressorDetails')}
              />
            </div>
          )}
        </FormSection>
        ...
        <FormSection
          ...
          description="Current ventilatory support"
        >
          <Checkbox
            label="On Mechanical Ventilation"
            description="Patient is currently intubated and ventilated"
            {...register('onMechanicalVent')}
          />
          {onMechanicalVent && (
            <div className="mt-4">
              <Select
        ...
        <FormSection title="Admission Notes">
          <Textarea
            ...
            placeholder="Any additional information relevant to admission..."
            fullWidth
            rows={4}
